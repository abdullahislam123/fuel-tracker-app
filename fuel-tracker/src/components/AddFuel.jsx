...
      if (!isNaN(l) && !isNaN(p)) newFormData.cost = (l * p).toFixed(2); // <--- CHANGE HERE (0 to 2)
    }
...
            'Authorization': token // 2. Token Bhejo
        },
        body: JSON.stringify({
          date: formData.date,
          time: formData.time,
          cost: formData.cost, // <--- CHANGE HERE (Directly use the fixed string value)
...
      <button onClick={handleSave} disabled={loading} className="w-full mt-8 py-4 text-white font-bold bg-emerald-500 rounded-xl hover:bg-emerald-600 disabled:bg-gray-400 transition-all">
