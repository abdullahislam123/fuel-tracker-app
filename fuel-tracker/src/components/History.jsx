...
const History = () => {
  ...
  const handleEditChange = (e) => {
    ...
    if (name === "liters" || name === "pricePerLiter") {
      ...
      if (!isNaN(l) && !isNaN(p)) newData.cost = (l * p).toFixed(0);
    }
    setEditData(newData);
  };
          ...
          <div key={entry._id} className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden relative transition-transform hover:scale-[1.01]">
            ...
            <div className="p-5 pl-7">
              ...
              <div className="mb-4">
                ...
                <h2 className="text-2xl font-extrabold text-slate-800">Rs. {parseInt(entry.cost).toLocaleString()}</h2>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-md rounded-2xl p-6 shadow-2xl animate-fade-in">
            ...
            <div className="space-y-4">
              ...
              <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Total Cost (Auto)</label><input type="number" name="cost" value={editData.cost} readOnly className="w-full p-2.5 bg-emerald-50 border border-emerald-100 text-emerald-700 rounded-lg font-bold" /></div>
